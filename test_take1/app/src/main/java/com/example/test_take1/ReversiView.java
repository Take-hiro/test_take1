...
import com.example.test_take1.model.Board;
import com.example.test_take1.model.Cell;
...
public class ReversiView extends View {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        float x = event.getX();
        float y = event.getY();
        if(mBoard.getCheckCell(0,0) == Cell.PUT_STATUS.None) mBoard.setCheckCell(mBoard.getTurn());
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_UP:
                ...
                if (r < Board.ROWS && c < Board.COLS) {
                    try {
                        if (mBoard.getCheckCell(r, c)== Cell.PUT_STATUS.Can) {
                            mBoard.changeCell(r, c, mBoard.getTurn());
                            mBoard.reversingCell(r, c, mBoard.getTurn());
                            mBoard.changeTurn();
                        }
                    } catch (Exception e) {
